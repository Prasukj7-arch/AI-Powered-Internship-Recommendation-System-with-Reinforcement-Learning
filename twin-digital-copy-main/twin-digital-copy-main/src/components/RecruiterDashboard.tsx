...
const RecruiterDashboard: React.FC = () => {
  ...
  const fetchApplications = async () => {
    try {
      const response = await fetch(
        "http://localhost:8000/api/recruiter/applications"
      );
  ...
  const fetchDashboardStats = async () => {
    try {
      const response = await fetch(
        "http://localhost:8000/api/recruiter/dashboard"
      );
      const data = await response.json();
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/recruiter/application/${selectedApplication.id}/review`,
        {
          method: "POST",
          headers: {
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setFeedback((prev) => ({ ...prev, decision: value }))
                    }
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFeedback((prev) => ({
                        ...prev,
                        recommendation_score: parseInt(e.target.value) || 5,
                      }))
                    }
                  />
                </div>
                      ...
                      {feedback.areas_for_improvement.map((area, index) => (
                        <div key={index} className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              removeFromArray("areas_for_improvement", index)
                            }
                          >
                            ×
                          </Button>
